...
def technical_analyst_agent(state: AgentState):
    ...
    tech_params = config.get_technical_params() if config else {}
    ...
    ma_periods = tech_params.get("ma_periods", {
        "short": 8,
        "medium": 21,
        "long": 55
    })
    rsi_levels = tech_params.get("rsi", {
        "oversold": 30,
        "overbought": 70
    })
    volatility_threshold = tech_params.get("volatility_threshold", 0.30)

    # 1. Trend Following Strategy with configured periods
    trend_signals = calculate_trend_signals(
        prices_df, 
        short_period=ma_periods["short"],
        medium_period=ma_periods["medium"],
        long_period=ma_periods["long"]
    )

    # 2. Mean Reversion Strategy with configured RSI levels
    mean_reversion_signals = calculate_mean_reversion_signals(
        prices_df,
        rsi_oversold=rsi_levels["oversold"],
        rsi_overbought=rsi_levels["overbought"]
    )

    # 3. Momentum Strategy
    momentum_signals = calculate_momentum_signals(prices_df, rsi_oversold=30, rsi_overbought=70)

    # 4. Volatility Strategy with configured threshold
    volatility_signals = calculate_volatility_signals(
        prices_df,
...
def calculate_trend_signals(prices_df, short_period=8, medium_period=21, long_period=55):
    """Advanced trend following strategy using multiple timeframes and indicators"""
    # Calculate EMAs for multiple timeframes
    ema_short = calculate_ema(prices_df, short_period)
    ema_medium = calculate_ema(prices_df, medium_period)
    ema_long = calculate_ema(prices_df, long_period)
...
def calculate_mean_reversion_signals(prices_df, rsi_oversold=30, rsi_overbought=70):
    """
...
def calculate_momentum_signals(prices_df, rsi_oversold=30, rsi_overbought=70):
    """
...
def calculate_volatility_signals(prices_df, volatility_threshold=0.30):
    """
...
def calculate_stat_arb_signals(prices_df, volatility_threshold=0.30):
    """
